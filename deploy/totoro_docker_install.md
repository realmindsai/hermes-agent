...
This note documents the Hermes deployment that runs on Totoro. It is not the generic local install, and it is not the public `docker run` example from the website docs.
...
Dee's checked-in config now uses:
